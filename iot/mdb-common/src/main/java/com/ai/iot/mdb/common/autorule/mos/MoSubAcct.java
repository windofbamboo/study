...
import com.ai.iot.mdb.common.device.AccountMo;
...
/**
 * 同一个key的关键字段表，用于获取key信息，同时也是业务表
 * @author qianwx
 */
public class MoSubAcct extends MoBase implements MoLoadable, Serializable {
  private static final long serialVersionUID = 2686423432348455417L;
  private static Logger logger = LoggerFactory.getLogger(MoSubAcct.class);
...
public long getAcctId() {
  ...
  @Override
  public byte[] getKey() {
    return generateKey(RedisConst.AutoRuleMdbKey.MDB_KEY_SUB_ACCT, subAcctId);
  }

  @Override
  public void setKeyId(String keyId) {
      subAcctId = Long.parseLong(keyId);
  }

  @Override
  public String getField() {
    return TABLE_ID.AUTORULE.TF_F_SUB_ACCT_HASHKEY;
  }
  ...
  public TotalInfo getTotalInfo(Connection conn, QueryRunner qr) {
    ...
    try {
      totalInfo = BaseDao.selectOne(qr, conn, "InfoloadAutoruleMapper.getSubAcctMoTotalInfo");
    } catch (Exception e) {
      logger.error("getSubAcctMoTotalInfo err ", e);
    }
    return totalInfo;
  }
  ...
  public List<KeyId> fetchBatchIds(Connection conn, QueryRunner qr, SegInfo segInfo) {
    return BaseDao.selectList(qr, conn, "InfoloadAutoruleMapper.getSubAcctIdSeg", segInfo.getMinId(), segInfo.getMaxId());
  }
  ...
  public MoBaseRegisterFactory.MdbKeyDispTypeEnum getKeyDispType() {
    return MoBaseRegisterFactory.MdbKeyDispTypeEnum.DISP_TYPE_SUB_ACCT;
  }

  //本MO从物理库加载的具体实现。单条
  @Override
  public MoBase loadDataFromPdb(Connection conn, String id, QueryRunner qr) {
    return BaseDao.selectOne(qr, conn, "InfoloadAutoruleMapper.getMdbSubAcctBySubAcctId", id);
  }

  //本MO从物理库加载的具体实现。多条
  @Override
  public List<MoBase> loadDataListFromPdb(Connection conn, String id, QueryRunner qr) {
    return BaseDao.selectList(qr, conn, "InfoloadAutoruleMapper.getMdbSubAcctBySubAcctId", id);
  }
}
