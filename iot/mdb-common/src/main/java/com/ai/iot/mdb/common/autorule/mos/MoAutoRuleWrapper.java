...
/**
 * 同一个key的关键字段表，用于获取key信息，不是业务表
 * @author qianwx
 */
public class MoAutoRuleWrapper extends MoBase implements MoLoadable, Serializable {
  private static final long serialVersionUID = 2686423432348455417L;
  private static Logger logger = LoggerFactory.getLogger(MoAutoRuleWrapper.class);
  private long acctId;
  ...
  @Override
  public byte[] getKey() {
    return generateKey(TABLE_ID.AUTORULE.TF_A_AUTO_RULE, acctId);
  }
  ...
  public TotalInfo getTotalInfo(Connection conn, QueryRunner qr) {
    ...
    try {
      totalInfo = BaseDao.selectOne(qr, conn, "InfoloadAutoruleMapper.getAutoruleMoTotalInfo");
    } catch (Exception e) {
      logger.error("getAutoruleMoTotalInfo err ", e);
    }
    return totalInfo;
  }
  ...
  public List<KeyId> fetchBatchIds(Connection conn, QueryRunner qr, SegInfo segInfo) {
    return BaseDao.selectList(qr, conn, "InfoloadAutoruleMapper.getAcctIdSeg", segInfo.getMinId(), segInfo.getMaxId());
  }
  ...
  public MoBase loadDataFromPdb(Connection conn, String id, QueryRunner qr) {
    this.acctId = Long.valueOf(id);
    MoAutoRuleWrapper moAutoRuleWrapper = new MoAutoRuleWrapper();
    moAutoRuleWrapper.setKeyId(id);
    return moAutoRuleWrapper;
  }

  //本MO从物理库加载的具体实现。多条
  @Override
  public List<MoBase> loadDataListFromPdb(Connection conn, String id, QueryRunner qr) {
    return BaseDao.selectList(qr, conn, "InfoloadAutoruleMapper.getMdbAutoruleByAcctId", id);//当前的mo可以不需要
  }
}
